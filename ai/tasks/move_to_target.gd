@tool
extends BTAction

# Task parameters.
@export var movementSpeed: float = 500

var character_body: UnitCore = null

## Note: Each method declaration is optional.
## At minimum, you only need to define the "_tick" method.


# Called to generate a display name for the task (requires @tool).
func _generate_name() -> String:
	return "Move to target"


# Called to initialize the task.
func _setup() -> void:
	character_body = self.agent as UnitCore


# Called when the task is entered.
func _enter() -> void:
	pass
	## Force stop character
	#if not character_body.velocity.is_zero_approx():
		#character_body.isRunning = true
	#else:
		#character_body.isRunning = false


# Called when the task is exited.
#func _exit() -> void:
	#character_body.isAttacking = false


# Called each time this task is ticked (aka executed).
func _tick(_delta: float) -> Status:
	var movement_component: MovementComponent = character_body.movement_component as MovementComponent
	if not is_instance_valid(movement_component):
		return FAILURE

	# checked above
	
	var target: UnitCore = blackboard.get_var("current_target")
	movement_component.move_to(target.global_position)
	return SUCCESS

# Strings returned from this method are displayed as warnings in the editor.
func _get_configuration_warnings() -> PackedStringArray:
	var warnings := PackedStringArray()
	return warnings
